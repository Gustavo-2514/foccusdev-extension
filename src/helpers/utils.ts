...
const getApiKey = async (context: vscode.ExtensionContext): Promise<string | undefined> => {
    try {
        ...
        if (validateApiKey(apiKey)) return apiKey;
            ...
            vscode.window.showErrorMessage("The provided API key is invalid. check: https://foccusdev.xyz/api-key");
...
const validateApiKey = (apiKey: string): boolean => {
    const regex = new RegExp('^foccus-[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$');
